    ...
    public function ajax(): JsonResponse
    {
        $data = $this->table
            ...
            ->editColumn('customer_id', function (Payment $item) {
                ...
                if ($item->order_type == \Botble\Courses\Models\CourseBooking::class) {
                    $courseBooking = \Botble\Courses\Models\CourseBooking::find($item->order_id);
                    if ($courseBooking && $courseBooking->address) {
                        return $courseBooking->address->full_name;
                    }
                }

                if ($item->order_type == \Botble\Hotel\Models\Booking::class) {
                    $booking = \Botble\Hotel\Models\Booking::find($item->order_id);
                    if ($booking && $booking->address) {
                        return $booking->address->full_name ?? $booking->address->name;
                    }
                }
