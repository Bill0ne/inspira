@php
    ...
    $defaultTaxPercentage = $defaultTaxPercentage ?? 0;
    $taxDisplay = number_format($selectedTaxPercentage ?? 0, 2, $decimalSeparator, $thousandSeparator);
@endphp
<div class="text-muted small course-price-helper" data-tax-rates='@json($taxRates)' data-currency='@json($currencyFormat)' data-price-field="#price" data-tax-field="#tax_id" data-selected-tax="{{ $selectedTaxId }}" data-default-tax="{{ $defaultTaxPercentage }}">
