...
class CoursePerformanceService
{
    ...
    protected int $maxViewsReference = 500;
    ...
    public function forCourse(Course $course): array
    {
        ...
        if (! isset($this->cache[$cacheKey])) {
            $views = $this->resolveViews($course);
    ...
    public function forSession(CourseSession $session): array
    {
        ...
        if (! isset($this->cache[$cacheKey])) {
            ...
            $views = $course ? $this->resolveViews($course) : 0;
    ...
    protected function resolveViews(?Course $course): int
    {
        if (! $course) {
            return 0;
        }
        ...
        if (! $slug) {
            return $this->viewCache[$courseId] = 0;
        }
        ...
        if (! $this->canQueryAnalytics) {
            return $this->viewCache[$courseId] = (int) $course->getMetaData('views', true) ?: 0;
        }

        try {
            $result = Analytics::dateRange(Period::days(30))
            ...
            $views = (int) Arr::get($result->first() ?? [], 'screenPageViews', 0);
        } catch (InvalidConfiguration|Throwable) {
            $views = 0;
        }

        return $this->viewCache[$courseId] = $views;
    }
}
