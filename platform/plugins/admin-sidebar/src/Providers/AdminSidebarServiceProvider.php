...
class AdminSidebarServiceProvider extends ServiceProvider
{
    ...
    protected function refreshMenuCacheIfNeeded(): void
    {
        $markerPath = storage_path('app/admin-sidebar-menu-cache');
        $signature = md5(json_encode($this->menuGroups()));

        if (File::exists($markerPath) && trim((string) File::get($markerPath)) === $signature) {
            return;
        }

        File::ensureDirectoryExists(dirname($markerPath));
        File::put($markerPath, $signature);
