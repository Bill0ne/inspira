    ...
    CanResetPasswordContract
{
    ...
    {
        return $this->hasMany(\Botble\Courses\Models\CourseBooking::class, 'customer_id');
    }

    public function hasBookedCourse(Course $course): bool
    {
        $bookings = $this->courseBookings;
        $coursesBooked = [];

        foreach ($bookings as $booking) {
            $coursesBooked[] = $booking->course->getKey();
        }

        return in_array($course->getKey(), $coursesBooked);
    }
    ...
    public function canReviewCourse(Course $course): bool
    {
        if (! auth('customer')->check()) {
            return false;
        }

        if (! $this->hasBookedCourse($course)) {
            return false;
        }

        if ($this->hasReviewedCourse($course)) {
            return false;
        }
